...
gulp.task('scripts', function() {
  return gulp.src(paths.scripts)
	...
	.pipe(plugins.concat('baasic-angular-metering.js'))
	.pipe(plugins.header('/*\n Baasic AngularJS Metering %%GULP_INJECT_VERSION%%\n (c) 2014-' + new Date().getFullYear() + ' Mono Ltd.  http://baasic.com\n License: MIT\n*/\n(function (angular, undefined) {\n')) 
	.pipe(plugins.footer('\n})(angular);'))
    .pipe(injectVersion())
	.pipe(plugins.beautify())
	.pipe(gulp.dest('dist'))
	.pipe(plugins.uglify({output: {comments: /^!|License: MIT/i}}))
	.pipe(plugins.rename('baasic-angular-metering.min.js'))
	.pipe(gulp.dest('dist'));
});

gulp.task('docs', function() {
  docgen.generateBaasicDocs('src', 'wiki', 'Baasic Metering Navigation', ['config.js'], ['home.md']);
});
